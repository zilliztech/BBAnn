...
void build_graph(const std::string &index_path,
    ...
    if (use_hnsw_sq) {
        ...
        build_hnsw_sq(index_path, hnswM, hnswefC, metric_type);
    } else {
        std::cout << "build graph type: HNSW" << std::endl;
        build_hnsw(index_path, hnswM, hnswefC, metric_type);
    }
}
...
void build_bbann(const std::string &raw_data_bin_file,
  ...
  build_graph(output_path, hnswM, hnswefC, metric_type, false);
...
SEARCH_BBANN_ONLY_DECL(uint8_t, uint32_t, CMin)
SEARCH_BBANN_ONLY_DECL(uint8_t, uint32_t, CMax)
SEARCH_BBANN_ONLY_DECL(int8_t, int32_t, CMin)
