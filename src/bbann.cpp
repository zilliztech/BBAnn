...
void search_bbann(
    ...
    const uint32_t entry_size = code_size + sizeof(uint32_t);
    uint8_t *code;
    DATAT *vec = new DATAT[dim];
    ...
    for (int64_t i = 0; i < nq; ++i) {
        ...
        for (int64_t j = 0; j < nprobe; ++j) {
            ...
            for (uint32_t k = 0; k < entry_num; ++k) {
                // std::cout<<"block size"<<k<<std::endl;
                char *entry_begin = buf_begin + entry_size * k;
                code = reinterpret_cast<uint8_t *>(entry_begin);
                // for (int d =0; d < dim; d++) {
                //vec[d] = (DATAT)(code[d]);
