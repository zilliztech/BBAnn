...
void split_raw_data(const std::string &raw_data_file,
                    const std::string &index_output_path, float *centroids,
  ...
  for (auto i = 0; i < K1; i++) {
    std::string cluster_raw_data_file_name =
        index_output_path + CLUSTER + std::to_string(i) + RAWDATA + BIN;
    std::string cluster_ids_data_file_name =
        index_output_path + CLUSTER + std::to_string(i) + GLOBAL_IDS + BIN;
  ...
  for (auto i = 0; i < block_num; i++) {
    ...
    knn_1<CMax<DISTT, size_t>, DATAT, float>(
        block_buf, centroids, ep - sp, K1, dim, 1, dists.data(),
        cluster_id.data(), select_computer<DATAT, float, DISTT>(metric_type));
  ...
  {// validate files
      for (auto i = 0; i < K1; i ++) {
          std::string cluster_raw_data_file_name = index_output_path + CLUSTER +
  std::to_string(i) + RAWDATA + BIN; std::string cluster_ids_data_file_name =
  index_output_path + CLUSTER + std::to_string(i) + GLOBAL_IDS + BIN; uint32_t
  rrs, rrd, iis, iid; get_bin_metadata(cluster_raw_data_file_name, rrs, rrd);
...
template <typename DATAT, typename DISTT>
void train_clusters(
    const std::string &cluster_path, uint32_t &graph_nb, uint32_t &graph_dim,
  ...
  rc.RecordSection("pq quantizer save centroids into " + pq_centroids_file +
                   " done");
  ...
  for (auto i = 0; i < K1; i++) {
    ...
    std::string data_file =
        cluster_path + CLUSTER + std::to_string(i) + RAWDATA + BIN;
    // global id file, read by split order, write by buckets
    std::string ids_file =
        cluster_path + CLUSTER + std::to_string(i) + GLOBAL_IDS + BIN;
    // meta_file, record the size of each bucket in cluster i
    std::string meta_file =
        cluster_path + CLUSTER + std::to_string(i) + META + BIN;
    // pq_codebook_file, save codebook of each cluster
    std::string pq_codebook_file =
        cluster_path + CLUSTER + std::to_string(i) + PQ + CODEBOOK + BIN;
    ...
    knn_1<CMax<DISTT, uint32_t>, DATAT, float>(
        datai, centroids_i, cluster_size, K2, cluster_dim, 1, dists.data(),
        cluster_id.data(), select_computer<DATAT, float, DISTT>(metric_type));
    ...
    data_reader2.read((char *)datai,
                      cluster_size * cluster_dim * sizeof(DATAT));
    pq_quantizer->encode_vectors_and_save(precomputer_table, cluster_size,
                                          datai, pq_codebook_file);
...
void create_graph_index(const std::string &index_path, const int hnswM,
                        const int hnswefC, MetricType metric_type) {

  TimeRecorder rc("create_graph_index");
  std::cout << "parameters of create hnsw: M = " << hnswM
            << ", efConstruction = " << hnswefC << std::endl;
  float *pdata = nullptr;
...
template <typename DATAT, typename DISTT>
void build_disk_index(const std::string &raw_data_file,
                      const std::string &index_output_path, const int hnswM,
  ...
  std::cout << "read meta from " << raw_data_file << ", nb = " << nb
            << "dim = " << dim << std::endl;
  ...
  ProductQuantizer<CMax<DISTT, uint32_t>, DATAT, uint8_t> pq_quantizer(dim, PQM,
                                                                       PQnbits);
  ...
  split_raw_data<DATAT, DISTT>(raw_data_file, index_output_path, centroids,
                               metric_type);
  rc.RecordSection("split_raw_data done");
  train_clusters<DATAT, DISTT>(index_output_path, graph_nb, graph_dim,
                               &pq_quantizer, metric_type);
  rc.RecordSection("train_clusters done");

  create_graph_index(index_output_path, hnswM, hnswefC,
                     metric_type); // hard code hnsw
...
void search_disk_index_simple(const std::string &index_path,
                              ...
                              const int topk, const int refine_topk,
                              const int nprobe, const int PQM,
                              const int PQnbits, MetricType metric_type) {
  // for debug
  // int nprobe = 2;
  // int topk = 5;
  ...
  std::cout << "welcome to search_disk_index_simple, show args: "
            ...
            << " answer_bin_file: " << answer_bin_file << " topk: " << topk
            << " refine topk: " << refine_topk << " nprobe: " << nprobe
            << " PQM: " << PQM << " PQnbits: " << PQnbits << std::endl;
  ...
  // read_bin_file<float>(pq_centroids_file, pq_centroids, num_pq_centroids,
  // dim_pq_centroids);

  // if DATAT is uint8_t, distance type is uint32_t, force transfer to uint32_t
  // from float, size is the same
  ...
  std::cout << "hnsw load " << index_hnsw->cur_element_count << " points"
            << std::endl;

  // uint32_t PQM = dim_queries / dim_pq_centroids;

  for (auto i = 0; i < K1; i++) {
    IOReader meta_reader(index_path + CLUSTER + std::to_string(i) + META + BIN,
                         MEGABYTE * 10);
    uint32_t cluster_sizei,
        meta_dimi; // cluster_sizei is the number of buckets in cluster i
    ...
    // IOReader pq_codebook_reader(index_path + CLUSTER + std::to_string(i) + PQ
    // + CODEBOOK + BIN, MEGABYTE * 10);
    std::ifstream pq_codebook_reader(index_path + CLUSTER + std::to_string(i) +
                                         PQ + CODEBOOK + BIN,
                                     std::ios::binary | std::ios::in);
    uint32_t pq_codebook_sizei,
        pqmi; // pq_codebook_sizei is the number of vectors in cluster i
    ...
    assert(pqmi == PQM);
    pq_codebook[i].resize(pq_codebook_sizei * pqmi);
    pq_codebook_reader.read((char *)pq_codebook[i].data(),
                            pq_codebook_sizei * pqmi * sizeof(uint8_t));
    num_base += pq_codebook_sizei;
    pq_codebook_reader.close();
  }
  std::cout << "load meta and pq_codebook done, num_base = " << num_base
            << std::endl;
...
#pragma omp parallel for
  ...
  ProductQuantizer<CMax<DISTT, uint64_t>, DATAT, uint8_t> pq_quantizer(
      dim_queries, PQM, PQnbits);
...
#pragma omp for
    for (auto i = 0; i < num_queries; i++) {
      pq_quantizer.calc_precompute_table(precompute_table,
                                         pquery + i * dim_queries, pq_cmp);
      ...
      for (auto j = 0; j < nprobe; j++) {
        ...
        pq_quantizer.search(precompute_table, pquery + i * dim_queries,
                            pq_codebook[cid].data() + off * PQM,
                            metas[cid][bid], refine_topk, pq_distancei,
                            pq_offseti, pq_cmp, j + 1 == nprobe, j == 0, cid,
                            off, i);
      }
      // std::cout << "query " << i << " has checked " << valid_bucket_cnt << "
      // buckets, checked " << valid_vector_cnt << " vectors." << std::endl;
    }
  ...
  for (auto i = 0; i < num_queries; i++) {
    ...
    // std::cout << "query " << i << " has " << valid_cnt << " vectors." <<
    // std::endl;
  }
...
#pragma omp parallel for
  for (auto i = 0; i < K1; i++) {
    ...
    raw_data_file_handlers[i].seekg(meta_bytes +
                                    pre_off * dim_queries * sizeof(DATAT));
    raw_data_file_handlers[i].read((char *)data_bufi,
                                   dim_queries * sizeof(DATAT));
    ...
    // for debug
    // std::cout << "load refine data of query " << refine_records[i][0].second
    // << " at pos: " << pre_off << std::endl;
    for (auto j = 0; j < refine_records[i].size(); j++) {
      if (refine_records[i][j].first != pre_off) {
        ...
        raw_data_file_handlers[i].read((char *)data_bufi,
                                       dim_queries * sizeof(DATAT));
        ids_data_file_handlers[i].seekg(meta_bytes +
                                        pre_off * sizeof(uint32_t));
        ids_data_file_handlers[i].read((char *)&global_id, sizeof(uint32_t));
        assert(global_id >= 0);
        assert(global_id < num_base);
        // std::cout << "load refine data of query " <<
        // refine_records[i][j].second << " at pos: " << pre_off << std::endl;
      }
      uint32_t qid = refine_records[i][j].second;
      auto dis =
          dis_computer(data_bufi, pquery + qid * dim_queries, dim_queries);
      std::unique_lock<std::mutex> lk(mtx[qid]);
      if (heap_comare_class::cmp(answer_dists[topk * qid], dis)) {
        heap_swap_top<heap_comare_class>(topk, answer_dists + topk * qid,
                                         answer_ids + topk * qid, dis,
                                         global_id);
      }
      // std::cout << "in cluster " << i << " calculate the refine dis at pos "
      // << pre_off << " for query " << qid << std::endl;
    }
...
template void search_disk_index_simple<float, float>(
    const std::string &index_path, const std::string &query_bin_file,
    const std::string &answer_bin_file, const int topk, const int refine_topk,
    const int nprobe, const int PQM, const int PQnbits,
    MetricType metric_type = MetricType::L2);

template void search_disk_index_simple<uint8_t, uint32_t>(
    const std::string &index_path, const std::string &query_bin_file,
    const std::string &answer_bin_file, const int topk, const int refine_topk,
    const int nprobe, const int PQM, const int PQnbits,
    MetricType metric_type = MetricType::L2);

template void build_disk_index<float, float>(
    const std::string &raw_data_file, const std::string &index_output_path,
    const int hnswM, const int hnswefC, const int PQM, const int PQnbits,
    MetricType metric_type = MetricType::L2);

template void build_disk_index<uint8_t, uint32_t>(
    const std::string &raw_data_file, const std::string &index_output_path,
    const int hnswM, const int hnswefC, const int PQM, const int PQnbits,
    MetricType metric_type = MetricType::L2);

template void split_raw_data<float, float>(const std::string &raw_data_file,
                                           const std::string &index_output_path,
                                           float *centroids,
                                           MetricType metric_type);

template void
split_raw_data<uint8_t, uint32_t>(const std::string &raw_data_file,
                                  const std::string &index_output_path,
                                  float *centroids, MetricType metric_type);

template void train_clusters<float, float>(
    const std::string &cluster_path, uint32_t &graph_nb, uint32_t &graph_dim,
    ProductQuantizer<CMax<float, uint32_t>, float, uint8_t> *pq_quantizer,
    MetricType metric_type);

template void train_clusters<uint8_t, uint32_t>(
    const std::string &cluster_path, uint32_t &graph_nb, uint32_t &graph_dim,
