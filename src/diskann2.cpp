...
void divide_raw_data(const std::string &raw_data_bin_file,
  ...
  for (int64_t i = 0; i < block_num; i++) {
    ...
    std::cout << "split the " << i << "th block, start position = " << sp
    ...
    //    dists.data(), cluster_id.data(), L2sqr<const DATAT, const float,
    //    DISTT>);
...
template <typename DATAT, typename DISTT, typename HEAPT>
void conquer_clusters(const std::string &output_path, const int K1,
                      const double avg_len, const int threshold) {
  TimeRecorder rc("conquer clusters");
  std::cout << "conquer clusters parameters:" << std::endl;
  std::cout << " output_path: " << output_path
            << " vector avg length: " << avg_len << std::endl;
  ...
  for (int i = 0; i < K1; i++) {
    ...
    std::string meta_file =
        output_path + CLUSTER + std::to_string(i) + META + BIN;
    ...
    data_reader.read((char *)datai,
                     (uint64_t)cluster_size * cluster_dim * sizeof(DATAT));
    uint32_t *idsi = new uint32_t[(uint64_t)ids_size * ids_dim];
    ids_reader.read((char *)idsi,
                    (uint64_t)ids_size * ids_dim * sizeof(uint32_t));

    int64_t K2 = (cluster_size - 1) / threshold + 1;
    std::cout << "cluster-" << i << " will split into " << K2 << " buckets."
              << std::endl;
    ...
    elkan_L2_assign<>(datai, centroids_i, cluster_dim, cluster_size, K2,
    ...
    { // validate bucket size
      ...
      std::cout << "cluster-" << i << " has " << empty_bkid.size()
                << " empty buckets:" << std::endl;
    ...
    // write_bin_file<float>(bucket_centroids_file, centroids_i, K2,
    // cluster_dim);
    bucket_ctd_writer.write((char *)centroids_i,
                            K2 * cluster_dim * sizeof(float));
    ...
    std::sort(cluster_off.begin(), cluster_off.end(),
    ...
    { // data_writer will close and auto flush in de-constructor
      ...
      for (int j = 0; j < cluster_size; j++) {
        ...
        data_writer.write((char *)(datai + ori_pos * cluster_dim),
                          sizeof(DATAT) * cluster_dim);
        ids_writer.write((char *)(idsi + ori_pos * ids_dim),
                         sizeof(uint32_t) * ids_dim);
        // for debug
        /*
        if (*(idsi + ori_pos * ids_dim) == 0) {
            std::cout << "vector0 is arranged to new pos: " << j << " in cluster
        " << i << std::endl; std::cout << " show vector0:" << std::endl; for
        (auto si = 0; si < cluster_dim; si ++) { std::cout << *(datai + ori_pos
        * cluster_dim + si) << " ";
            }
            std::cout << std::endl;
            v0cid = i;
    ...
    // read_bin_file<uint32_t>(meta_file, buckets_size.data(), meta_numi,
    // meta_dimi); assert(meta_numi == K2); assert(meta_dimi == 1);
    graph_nb += K2;
...
void build_graph(const std::string &index_path, const int hnswM,
  ...
  read_bin_file<uint64_t>(index_path + CLUSTER + COMBINE_IDS + BIN, pids, nids,
                          nidsdim);
...
void train_pq_quantizer(const std::string &raw_data_bin_file,
  ...
  for (int i = 0; i < K1; i++) {
    std::string data_file =
        output_path + CLUSTER + std::to_string(i) + RAWDATA + BIN;
    std::string pq_codebook_file =
        output_path + CLUSTER + std::to_string(i) + PQ + CODEBOOK + BIN;
    ...
    data_reader.read((char *)datai,
                     (uint64_t)cluster_size * cluster_dim * sizeof(DATAT));
    pq_quantizer.encode_vectors_and_save(precomputer_table, cluster_size, datai,
                                         pq_codebook_file);
    delete[] datai;
    rc.RecordSection("the " + std::to_string(i) +
                     "th cluster encode and save codebook done.");
  }
...
template <typename DATAT, typename DISTT, typename HEAPT>
void train_pq_residual_quantizer(const std::string &raw_data_bin_file,
                                 const std::string &output_path, const int K1,
                                 const int PQM, const int PQnbits,
                                 MetricType metric_type) {
  ...
  int64_t pq_sample_num =
      std::min(static_cast<int64_t>(65536),
               std::min(static_cast<int64_t>(nb * PQ_SAMPLE_RATE),
                        static_cast<int64_t>(std::accumulate(
                            metas[0].begin(), metas[0].end(), 0))));
  ...
  read_bin_file<float>(output_path + BUCKET + CENTROIDS + BIN, ivf_cen, ivf_n,
                       ivf_dim);
  assert(ivf_dim == dim);
  assert(ivf_cen != nullptr);

  reservoir_sampling_residual<DATAT>(output_path, metas, ivf_cen, dim,
                                     pq_sample_num, sample_data, sample_ivf_cen,
                                     K1);
  ...
  for (int i = 0; i < K1; ++i) {
    ...
    quantizer.encode_vectors_and_save(precompute_table, cluster_size, datai,
                                      ivf_cen + bucket_cnt * cluster_dim,
                                      metas[i], pq_codebook_file);
...
// page alignment 4 raw data and uid in each cluster 4 better refine
// performance, uid follow after rawdata
template <typename DATAT>
void page_align(const std::string &raw_data_bin_file,
                const std::string &output_path, const int K1) {
  ...
  std::cout << "page size = " << page_size
            << ", number of nodes(vector + id) in per page: " << nnpp
            << std::endl;
  ...
  for (uint32_t i = 0; i < K1; i++) {
    ...
    std::string cluster_ids_data_file_name =
        output_path + CLUSTER + std::to_string(i) + GLOBAL_IDS + BIN;
    std::string cluster_raw_data_file_name2 =
        output_path + CLUSTER + std::to_string(i) + "-" + RAWDATA + BIN;
    uint32_t cluster_size, cluster_dim, ids_size, ids_dim;
    ...
    std::cout << "there are " << npn << " pages 4 nodes in cluster "
    ...
    *(uint32_t *)(data_page_buf + 5 * sizeof(uint32_t)) = i;
    ...
    if (rm_dat_ret == 0) {
      std::cout << "remove old raw data file: " << cluster_raw_data_file_name
                << " success." << std::endl;
    } else {
      std::cout << "remove old raw data file: " << cluster_raw_data_file_name
                << " failed." << std::endl;
    ...
    if (rm_ids_ret == 0) {
      std::cout << "remove old ids data file: " << cluster_ids_data_file_name
                << " success." << std::endl;
    } else {
      std::cout << "remove old ids data file: " << cluster_ids_data_file_name
                << " failed." << std::endl;
...
void build_bigann(const std::string &raw_data_bin_file,
                  ...
                  const int hnswefC, const int PQM, const int PQnbits,
                  const int K1, const int threshold, MetricType metric_type,
  ...
  std::cout << " raw_data_bin_file: " << raw_data_bin_file
            ...
            << " hnsw.efConstruction: " << hnswefC << " PQ.M: " << PQM
            << " PQ.nbits: " << PQnbits << " K1: " << K1
            << " bucket split threshold: " << threshold << std::endl;
  ...
  if (QuantizerType::PQ == quantizer_type) {
    train_pq_quantizer<DATAT, DISTT, HEAPT>(raw_data_bin_file, output_path, K1,
                                            PQM, PQnbits, metric_type);
  } else if (QuantizerType::PQRES == quantizer_type) {
    train_pq_residual_quantizer<DATAT, DISTT, HEAPT>(
        raw_data_bin_file, output_path, K1, PQM, PQnbits, metric_type);
  }
...
void load_pq_codebook(const std::string &index_path,
  ...
  std::cout << " index path: " << index_path << " K1: " << K1 << std::endl;

  for (int i = 0; i < K1; i++) {
    std::ifstream reader(index_path + CLUSTER + std::to_string(i) + PQ +
                             CODEBOOK + BIN,
                         std::ios::binary);
    uint32_t sizei, dimi;
    ...
    reader.read((char *)pq_codebook[i].data(),
                (uint64_t)sizei * dimi * sizeof(uint8_t));
    reader.close();
  }
...
void load_meta(const std::string &index_path,
               std::vector<std::vector<uint32_t>> &meta, const int K1) {
...
void search_pq_quantizer(ProductQuantizer<HEAPTT, DATAT, uint8_t> &pq_quantizer,
                         const uint32_t nq, const uint32_t dq,
                         uint64_t *buckets_label, const int nprobe,
                         const int refine_topk, const int K1,
                         const DATAT *pquery,
                         std::vector<std::vector<uint8_t>> &pq_codebook,
                         std::vector<std::vector<uint32_t>> &meta,
                         DISTT *&pq_distance, uint64_t *&pq_offsets) {
  TimeRecorder rc("search quantizer");
  std::cout << "search quantizer parameters:" << std::endl;
  std::cout << " pq_quantizer:" << &pq_quantizer << " nq: " << nq
            << " dq: " << dq << " buckets_label: " << buckets_label
            << " nprobe: " << nprobe << " refine_topk: " << refine_topk
            << " K1: " << K1 << " pquery: " << static_cast<const void *>(pquery)
            << " pq_distance: " << static_cast<void *>(pq_distance)
            << " pq_offsets: " << static_cast<void *>(pq_offsets) << std::endl;
    ...
    for (int i = 0; i < nq; i++) {
      ...
      for (int j = 0; j < nprobe; j++) {
        ...
        pq_quantizer.search(precompute_table, pquery + i * dq,
                            pq_codebook[cid].data() + (int64_t)off * pqm,
                            meta[cid][bid], refine_topk, pq_distancei,
                            pq_offseti, j + 1 == nprobe, j == 0, cid, off, i);
      }
    }
    delete[] precompute_table;
  }
...
#pragma omp parallel for
  for (auto i = 0; i < nq; i ++) {
      ProductQuantizer<HEAPTT, DATAT, uint8_t>
pq_quantizer_copiesi(pq_quantizer);
...
void search_pq_residual_quantizer(
    PQResidualQuantizer<HEAPTT, DATAT, uint8_t> &quantizer, const uint32_t nq,
    const uint32_t dq, float *ivf_centroids, uint64_t *buckets_label,
    const int nprobe, const int refine_topk, const int K1, const DATAT *pquery,
    std::vector<std::vector<uint8_t>> &pq_codebook,
    std::vector<std::vector<uint32_t>> &meta, DISTT *&pq_distance,
    uint64_t *&pq_offsets) {
  TimeRecorder rc("search pq residual quantizer");
  std::cout << "search quantizer parameters:" << std::endl;
  std::cout << " quantizer:" << &quantizer << " nq: " << nq << " dq: " << dq
            << " buckets_label: " << buckets_label << " nprobe: " << nprobe
            << " refine_topk: " << refine_topk << " K1: " << K1
            << " pquery: " << static_cast<const void *>(pquery)
            << " pq_distance: " << static_cast<void *>(pq_distance)
            << " pq_offsets: " << static_cast<void *>(pq_offsets) << std::endl;
    ...
    for (int64_t i = 0; i < nq; i++) {
      ...
      for (auto j = 0; j < nprobe; j++) {
        ...
        quantizer.search(precompute_table, pquery + i * dq, cen,
                         ...
                         meta[cid][bid], refine_topk, pq_distancei, pq_offseti,
                         j + 1 == nprobe, j == 0, cid, off, i);
      }
    }
    delete[] precompute_table;
...
// cannot work
template <typename DATAT, typename DISTT, typename HEAPT>
void refine(const std::string &index_path, const int K1, const uint32_t nq,
            const uint32_t dq, const int topk, const int refine_topk,
            uint64_t *pq_offsets, const DATAT *pquery, DISTT *&answer_dists,
            uint32_t *&answer_ids,
            Computer<DATAT, DATAT, DISTT> &dis_computer) {
  TimeRecorder rc("refine");
  std::cout << "refine parameters:" << std::endl;
  std::cout << " index_path: " << index_path << " cluster size: " << K1
            << " number of query: " << nq << " dim of query: " << dq
            << " topk: " << topk << " refine_topk:" << refine_topk
            << " pq_offsets:" << static_cast<void *>(pq_offsets)
            << " pquery:" << static_cast<const void *>(pquery)
            << " answer_dists:" << static_cast<void *>(answer_dists)
            << " answer_ids:" << static_cast<void *>(answer_ids) << std::endl;
  ...
  for (int i = 0; i < K1; i++) {
    std::string data_filei =
        index_path + CLUSTER + std::to_string(i) + RAWDATA + BIN;
    std::string ids_filei =
        index_path + CLUSTER + std::to_string(i) + GLOBAL_IDS + BIN;
    ...
    std::cout << "cluster-" << i << " has " << clu_size << " vectors,"
              ...
              << " clu_id_dim = " << clu_id_dim << std::endl;
  }
...
#pragma omp parallel for
  for (int i = 0; i < K1; i++) {
    if (refine_records[i].size() == 0)
      continue;
    std::sort(refine_records[i].begin(), refine_records[i].end(),
              [](const auto &l, const auto &r) { return l.first < r.first; });
    ...
    for (int j = 0; j < refine_records[i].size(); j++) {
      if (refine_records[i][j].first != pre_off) {
        ...
        raw_data_file_handlers[i].seekg(meta_bytes +
                                            pre_off * dq * sizeof(DATAT),
                                        raw_data_file_handlers[i].beg);
        raw_data_file_handlers[i].read((char *)data_bufi, dq * sizeof(DATAT));
        ids_data_file_handlers[i].seekg(meta_bytes + pre_off * sizeof(uint32_t),
                                        ids_data_file_handlers[i].beg);
      ...
      if (HEAPT::cmp(answer_dists[topk * qid], dis)) {
        heap_swap_top<HEAPT>(topk, answer_dists + topk * qid,
                             answer_ids + topk * qid, dis, global_id);
      }
    }
  ...
  for (int i = 0; i < K1; i++) {
    std::cout << "cluster-" << i << ": " << load_vectors[i] << "/"
              << refine_topk << std::endl;
    tot += load_vectors[i];
  }
  std::cout << "total load refine vectors: " << tot << "/" << refine_topk * nq
            << std::endl;
...
template <typename DATAT, typename DISTT, typename HEAPT>
void aligned_refine(const std::string &index_path, const int K1,
                    const uint32_t nq, const uint32_t dq, const int topk,
                    const int refine_topk, uint64_t *pq_offsets,
                    const DATAT *pquery, DISTT *&answer_dists,
                    uint32_t *&answer_ids,
                    Computer<DATAT, DATAT, DISTT> &dis_computer) {
  ...
  std::vector<std::ifstream> raw_data_file_handlers(K1);
  for (int i = 0; i < K1; i++) {
    std::string aligned_data_filei =
        index_path + CLUSTER + std::to_string(i) + "-" + RAWDATA + BIN;
    raw_data_file_handlers[i] =
        std::ifstream(aligned_data_filei, std::ios::binary);
    ...
    assert(check == i);
    std::cout << "cluster-" << i << " has " << clu_size << " vectors,"
              << " clu_dim = " << clu_dim << std::endl;
    std::cout << "main meta: " << std::endl;
    std::cout << " number of nodes in per page: " << nnpp
              << " number of pages 4 node(rawdata + uid): " << npn << std::endl;
  }
  rc.RecordSection("open rawdata and idsdata file handlers");
...
#pragma omp parallel for
  for (int i = 0; i < K1; i++) {
    ...
    uint32_t global_id;
    raw_data_file_handlers[i].seekg((pn + 1) * page_size,
                                    raw_data_file_handlers[i].beg);
    ...
    for (int j = 0; j < refine_records[i].size(); j++) {
      ...
      uint32_t qid = refine_records[i][j].second;
      auto dis =
          dis_computer((DATAT *)(dat_bufi + (refine_off % nnpp) * node_size),
                       pquery + qid * dq, dq);
      std::unique_lock<std::mutex> lk(mtx[qid]);
      if (HEAPT::cmp(answer_dists[topk * qid], dis)) {
        heap_swap_top<HEAPT>(
            topk, answer_dists + topk * qid, answer_ids + topk * qid, dis,
            *((uint32_t *)(dat_bufi + (refine_off % nnpp) * node_size +
                           vector_size)));
      }
    }
  ...
  std::cout << "total refine vectors: " << (int64_t)nq * refine_topk
            << ", load vector pages: " << vector_load_tot
            << ", load ids pages: " << id_load_tot
            << ", vector page hit: " << vector_page_hit_tot
            << ", ids page hit: " << id_page_hit_tot
...
template <typename DATAT, typename DISTT, typename HEAPT>
void aligned_refine_c(const std::string &index_path, const int K1,
                      const uint32_t nq, const uint32_t dq, const int topk,
                      const int refine_topk, uint64_t *pq_offsets,
                      const DATAT *pquery, DISTT *&answer_dists,
                      uint32_t *&answer_ids,
                      Computer<DATAT, DATAT, DISTT> &dis_computer) {
  TimeRecorder rc("aligned aligned_refine_c with C-style interface: open(), "
                  "pread(), close(). Here with O_RDONLY | O_DIRECT");
  std::cout << "aligned refine parameters:" << std::endl;
  std::cout << " index_path: " << index_path << " cluster size: " << K1
            << " number of query: " << nq << " dim of query: " << dq
            << " topk: " << topk << " refine_topk:" << refine_topk
  ...
  char *dat_buf;
  int posix_memalign_rt = posix_memalign(reinterpret_cast<void **>(&dat_buf),
                                         O_DIRECT_ALIGNMENT, page_size);
  assert(posix_memalign_rt == 0);
  char *ids_buf;
  posix_memalign_rt = posix_memalign(reinterpret_cast<void **>(&ids_buf),
                                     O_DIRECT_ALIGNMENT, page_size);
  assert(posix_memalign_rt == 0);
  memset(dat_buf, 0, page_size);
  ...
  for (int i = 0; i < K1; i++) {
    std::string aligned_data_filei =
        index_path + CLUSTER + "-" + std::to_string(i) + RAWDATA + BIN;
    std::string aligned_ids_filei =
        index_path + CLUSTER + "-" + std::to_string(i) + GLOBAL_IDS + BIN;
#if DIRECTIO
    raw_data_file_fds.emplace_back(
        open(aligned_data_filei.c_str(), O_RDONLY | O_DIRECT));
    ids_data_file_fds.emplace_back(
        open(aligned_ids_filei.c_str(), O_RDONLY | O_DIRECT));
#else
    ...
    std::cout << " number of vectors in per page: " << nvpp
              ...
              << " number of pages 4 id: " << npi << std::endl;
  }
  rc.RecordSection("open rawdata and idsdata file handlers");
...
#pragma omp parallel for
  for (int i = 0; i < K1; i++) {
    ...
    char *dat_bufi;
    char *ids_bufi;
    int posix_memalign_rt = posix_memalign(reinterpret_cast<void **>(&dat_bufi),
                                           O_DIRECT_ALIGNMENT, page_size);
    assert(posix_memalign_rt == 0);
    posix_memalign_rt = posix_memalign(reinterpret_cast<void **>(&ids_bufi),
                                       O_DIRECT_ALIGNMENT, page_size);
    assert(posix_memalign_rt == 0);
    ...
    int pread_size =
        pread(raw_data_file_fds[i], dat_bufi, page_size, (pv + 1) * page_size);
    assert(pread_size != -1);
    pread_size =
        pread(ids_data_file_fds[i], ids_bufi, page_size, (pi + 1) * page_size);
    assert(pread_size != -1);
    ...
    for (int j = 0; j < refine_records[i].size(); j++) {
      ...
      if (refine_off >= (pv + 1) * nvpp) {
        pv = refine_off / nvpp;
        pread_size = pread(raw_data_file_fds[i], dat_bufi, page_size,
                           (pv + 1) * page_size);
        assert(pread_size != -1);
        refine_statastics[i].vector_load_cnt++;
      } else {
      ...
      if (refine_off >= (pi + 1) * nipp) {
        pi = refine_off / nipp;
        pread_size = pread(ids_data_file_fds[i], ids_bufi, page_size,
                           (pi + 1) * page_size);
        assert(pread_size != -1);
      ...
      uint32_t qid = refine_records[i][j].second;
      auto dis =
          dis_computer((DATAT *)(dat_bufi + (refine_off % nvpp) * vector_size),
                       pquery + qid * dq, dq);
      std::unique_lock<std::mutex> lk(mtx[qid]);
      if (HEAPT::cmp(answer_dists[topk * qid], dis)) {
        heap_swap_top<HEAPT>(
            topk, answer_dists + topk * qid, answer_ids + topk * qid, dis,
            *((uint32_t *)(ids_bufi + (refine_off % nipp) * sizeof(uint32_t))));
      }
    }
...
void save_sift_answer(const std::string &answer_bin_file, const int topk,
...
template <typename DISTT, typename HEAPT>
void save_comp_answer(const std::string &answer_bin_file, const int topk,
                      const uint32_t nq, DISTT *&answer_dists,
                      uint32_t *&answer_ids) {
  std::ofstream answer_writer(answer_bin_file, std::ios::binary);
...
void save_answers(const std::string &answer_bin_file, const int topk,
                  const uint32_t nq, DISTT *&answer_dists,
                  uint32_t *&answer_ids, bool use_comp_format) {
  ...
  std::cout << " answer_bin_file: " << answer_bin_file << " topk: " << topk
            ...
            << " use_comp_format: " << use_comp_format << std::endl;

  auto f = use_comp_format ? save_comp_answer<DISTT, HEAPT>
                           : save_sift_answer<DISTT, HEAPT>;
...
void search_bigann(const std::string &index_path,
                   ...
                   const int refine_nprobe, const int topk,
                   ...
                   const int K1, std::vector<std::vector<uint8_t>> &pq_codebook,
                   std::vector<std::vector<uint32_t>> &meta,
                   Computer<DATAT, DATAT, DISTT> &dis_computer) {
  ...
  std::cout << " index_path: " << index_path
            ...
            << " refine_nprobe: " << refine_nprobe << " topk: " << topk
            << " refine topk: " << refine_topk << " K1: " << K1 << std::endl;
  ...
  search_graph<DATAT>(index_hnsw, nq, dq, nprobe, refine_nprobe, pquery,
                      p_labels);
  rc.RecordSection("search buckets done.");

  float *ivf_centroids = nullptr;
  uint32_t c_n, c_dim;
  read_bin_file<float>(index_path + BUCKET + CENTROIDS + BIN, ivf_centroids,
                       c_n, c_dim);
  search_pq_residual_quantizer<DATAT, DISTT, HEAPTT>(
      quantizer, nq, dq, ivf_centroids, p_labels, nprobe, refine_topk, K1,
      pquery, pq_codebook, meta, pq_distance, pq_offsets);
...
#endif
    // refine<DATAT, DISTT, HEAPT>(index_path, K1, nq, dq, topk, refine_topk,
    // pq_offsets, pquery, answer_dists, answer_ids, dis_computer);
    aligned_refine<DATAT, DISTT, HEAPT>(
        index_path, K1, nq, dq, topk, refine_topk, pq_offsets, pquery,
        answer_dists, answer_ids,
        dis_computer); // refine with C++ std::ifstream
    //    aligned_refine_c<DATAT, DISTT, HEAPT>(index_path, K1, nq, dq, topk,
    //    refine_topk, pq_offsets, pquery, answer_dists, answer_ids,
    //    dis_computer);  // refine_c with C open(), pread(), close()
  }
...
void search_bigann(const std::string &index_path,
  ...
  */
  ...
  search_pq_quantizer<DATAT, DISTT, HEAPTT>(
      pq_quantizer, nq, dq, p_labels, nprobe, refine_topk, K1, pquery,
      pq_codebook, meta, pq_distance, pq_offsets);
  ...
  {// for debug
      ...
      for (auto i = 0; i < nq; i ++) {
          ...
          for (auto j = 0; j < refine_topk; j ++) {
              ...
              uint32_t cid, off, qid;
              parse_refine_id(pq_offseti[j], cid, off, qid);
              std::cout << "(" << cid << ", " << off << ", " << qid << ", pqdis:
  " << pq_distancei[j] << ") " << std::endl;
          }
          std::cout << std::endl;
      }
  }
...
search_bigann<float, float, CMax<float, uint32_t>, CMax<float, uint64_t>>(
    ...
    const std::string &answer_bin_file, const int nprobe,
    const int refine_nprobe, const int topk, const int refine_topk,
...
template void search_bigann<uint8_t, uint32_t, CMax<uint32_t, uint32_t>,
                            CMax<uint32_t, uint64_t>>(
    const std::string &index_path, const std::string &query_bin_file,
    const std::string &answer_bin_file, const int nprobe,
    const int refine_nprobe, const int topk, const int refine_topk,
    std::shared_ptr<hnswlib::HierarchicalNSW<uint32_t>> index_hnsw,
    PQResidualQuantizer<CMax<uint32_t, uint64_t>, uint8_t, uint8_t>
        &pq_quantizer,
    const int K1, std::vector<std::vector<uint8_t>> &pq_codebook,
    std::vector<std::vector<uint32_t>> &meta,
    Computer<uint8_t, uint8_t, uint32_t> &dis_computer);

template void search_bigann<uint8_t, uint32_t, CMin<uint32_t, uint32_t>,
                            CMin<uint32_t, uint64_t>>(
    const std::string &index_path, const std::string &query_bin_file,
    const std::string &answer_bin_file, const int nprobe,
    const int refine_nprobe, const int topk, const int refine_topk,
    std::shared_ptr<hnswlib::HierarchicalNSW<uint32_t>> index_hnsw,
    PQResidualQuantizer<CMin<uint32_t, uint64_t>, uint8_t, uint8_t>
        &pq_quantizer,
    const int K1, std::vector<std::vector<uint8_t>> &pq_codebook,
    std::vector<std::vector<uint32_t>> &meta,
    Computer<uint8_t, uint8_t, uint32_t> &dis_computer);

template void search_bigann<int8_t, int32_t, CMax<int32_t, uint32_t>,
                            CMax<int32_t, uint64_t>>(
    const std::string &index_path, const std::string &query_bin_file,
    const std::string &answer_bin_file, const int nprobe,
    const int refine_nprobe, const int topk, const int refine_topk,
    std::shared_ptr<hnswlib::HierarchicalNSW<int32_t>> index_hnsw,
    PQResidualQuantizer<CMax<int32_t, uint64_t>, int8_t, uint8_t> &pq_quantizer,
    const int K1, std::vector<std::vector<uint8_t>> &pq_codebook,
    std::vector<std::vector<uint32_t>> &meta,
    Computer<int8_t, int8_t, int32_t> &dis_computer);

template void search_bigann<int8_t, int32_t, CMin<int32_t, uint32_t>,
                            CMin<int32_t, uint64_t>>(
    const std::string &index_path, const std::string &query_bin_file,
    const std::string &answer_bin_file, const int nprobe,
    const int refine_nprobe, const int topk, const int refine_topk,
    std::shared_ptr<hnswlib::HierarchicalNSW<int32_t>> index_hnsw,
...
template void save_answers<float, CMax<float, uint32_t>>(
    const std::string &answer_bin_file, const int topk, const uint32_t nq,
    float *&answer_dists, uint32_t *&answer_ids, bool use_comp_format = true);

template void save_answers<float, CMin<float, uint32_t>>(
    const std::string &answer_bin_file, const int topk, const uint32_t nq,
    float *&answer_dists, uint32_t *&answer_ids, bool use_comp_format = true);

template void save_answers<uint32_t, CMax<uint32_t, uint32_t>>(
    const std::string &answer_bin_file, const int topk, const uint32_t nq,
    uint32_t *&answer_dists, uint32_t *&answer_ids,
    bool use_comp_format = true);

template void save_answers<uint32_t, CMin<uint32_t, uint32_t>>(
    const std::string &answer_bin_file, const int topk, const uint32_t nq,
    uint32_t *&answer_dists, uint32_t *&answer_ids,
    bool use_comp_format = true);

template void save_answers<int32_t, CMax<int32_t, uint32_t>>(
    const std::string &answer_bin_file, const int topk, const uint32_t nq,
    int32_t *&answer_dists, uint32_t *&answer_ids, bool use_comp_format = true);

template void save_answers<int32_t, CMin<int32_t, uint32_t>>(
    const std::string &answer_bin_file, const int topk, const uint32_t nq,
    int32_t *&answer_dists, uint32_t *&answer_ids, bool use_comp_format = true);

template void
search_graph<float>(std::shared_ptr<hnswlib::HierarchicalNSW<float>> index_hnsw,
                    const int nq, const int dq, const int nprobe,
                    const int refine_nprobe, const float *pquery,
                    uint64_t *buckets_label);

template void search_graph<uint8_t>(
    std::shared_ptr<hnswlib::HierarchicalNSW<float>> index_hnsw, const int nq,
    const int dq, const int nprobe, const int refine_nprobe,
    const uint8_t *pquery, uint64_t *buckets_label);

template void search_graph<int8_t>(
    std::shared_ptr<hnswlib::HierarchicalNSW<float>> index_hnsw, const int nq,
    const int dq, const int nprobe, const int refine_nprobe,
    const int8_t *pquery, uint64_t *buckets_label);

template void build_bigann<float, float, CMax<float, uint32_t>>(
...
template void build_bigann<float, float, CMin<float, uint32_t>>(
    const std::string &raw_data_bin_file, const std::string &output_path,
    const int hnswM, const int hnswefC, const int PQM, const int PQnbits,
    const int K1, const int threshold, MetricType metric_type,
    QuantizerType quantizer_type);

template void build_bigann<uint8_t, uint32_t, CMax<uint32_t, uint32_t>>(
    const std::string &raw_data_bin_file, const std::string &output_path,
    const int hnswM, const int hnswefC, const int PQM, const int PQnbits,
    const int K1, const int threshold, MetricType metric_type,
    QuantizerType quantizer_type);

template void build_bigann<uint8_t, uint32_t, CMin<uint32_t, uint32_t>>(
    const std::string &raw_data_bin_file, const std::string &output_path,
    const int hnswM, const int hnswefC, const int PQM, const int PQnbits,
    const int K1, const int threshold, MetricType metric_type,
    QuantizerType quantizer_type);

template void build_bigann<int8_t, int32_t, CMax<int32_t, uint32_t>>(
    const std::string &raw_data_bin_file, const std::string &output_path,
    const int hnswM, const int hnswefC, const int PQM, const int PQnbits,
    const int K1, const int threshold, MetricType metric_type,
    QuantizerType quantizer_type);

template void build_bigann<int8_t, int32_t, CMin<int32_t, uint32_t>>(
    const std::string &raw_data_bin_file, const std::string &output_path,
    const int hnswM, const int hnswefC, const int PQM, const int PQnbits,
    const int K1, const int threshold, MetricType metric_type,
    QuantizerType quantizer_type);

template void train_cluster<float>(const std::string &raw_data_bin_file,
                                   const std::string &output_path, const int K1,
                                   float **centroids, double &avg_len);

template void train_cluster<uint8_t>(const std::string &raw_data_bin_file,
                                     const std::string &output_path,
                                     const int K1, float **centroids,
                                     double &avg_len);

template void train_cluster<int8_t>(const std::string &raw_data_bin_file,
                                    const std::string &output_path,
                                    const int K1, float **centroids,
                                    double &avg_len);
