...
void hierarchical_clusters() {
  ...
  for (uint32_t i = 0; i < 1; i++) {
    ...
    data_reader.read((char *)datai,
                     cluster_size * cluster_dim * sizeof(uint8_t));
    ...
    *(uint32_t *)(data_blk_buf + 5 * sizeof(uint32_t)) = i;
    ...
    recursive_kmeans<uint8_t>(i, 1000000, datai, idi, cluster_dim, threshold,
                              blk_size, blk_num, data_writer, centroids_writer,
                              centroids_id_writer, 0, false);
