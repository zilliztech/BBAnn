...
int main(int argc, char** argv) {
    ...
    if (buff == nullptr) printf("mall error\n");
    ...
    for (int i=0, batch_cnt=0; i<n; i+=BATCH, batch_cnt++) {
        ...
        if (loacl_batch != BATCH && i != 0) break;

        fread(buff, loacl_batch * dim, Type_Size[t], fi);

        double avg_len, max_len, min_len;
        if (t == FLOAT) {
            stat_length<float>((float*)buff, loacl_batch, dim, max_len, min_len, avg_len);
        } else if (t == UINT8) {
            stat_length<unsigned char>((unsigned char*)buff, loacl_batch, dim, max_len, min_len, avg_len);
        } else if (t == INT8) {
            stat_length<char>((char*)buff, loacl_batch, dim, max_len, min_len, avg_len);
        }

        printf("batch_i %d avg %lf min %lf max %lf\n", batch_cnt, avg_len, min_len, max_len);

        if (max_len > global_max_len) global_max_len = max_len;
        if (min_len < global_min_len) global_min_len = min_len;
        global_avg_len = (global_avg_len * batch_cnt + avg_len) / (batch_cnt + 1);
    }

    printf("stat %d * %d vectors:\n", n / BATCH * BATCH, dim);
    printf("avg len %lf\nmin len %lf\nmax len %lf\n", global_avg_len, global_min_len, global_max_len);
