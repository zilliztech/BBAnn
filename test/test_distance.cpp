...
void test_uint8_compute_lookuptable_ip() {
  ...
  for (int i = 0; i < nq; i++) {
    for (int j = 0; j < m; j++) {
      for (int k = 0; k < nb; k++) {
        result[j * 256 + k] = IP<uint8_t, float, float>(
            xq + i * j * d, xb + (j * 256 + k) * d, d);
      }
    }
  }
  cout << "normal search time" << elapsed() - t0 << endl;
  ...
  for (int i = 0; i < nq; i++)
    for (int j = 0; j < m; j++) {
      compute_lookuptable_IP<uint8_t>(xq + i * j * d, ytransform + j * 256 * d,
                                      result2 + j * 256, d, 256);
    }
...
void test_int8_compute_lookuptable_ip() {
  ...
  for (int i = 0; i < nq; i++) {
    for (int j = 0; j < m; j++) {
      for (int k = 0; k < nb; k++) {
        result[j * 256 + k] =
            IP<int8_t, float, float>(xq + i * j * d, xb + (j * 256 + k) * d, d);
      }
    }
  }
  cout << "normal search time" << elapsed() - t0 << endl;
  ...
  for (int i = 0; i < nq; i++)
    for (int j = 0; j < m; j++) {
      compute_lookuptable_IP<int8_t>(xq + i * j * d, ytransform + j * 256 * d,
                                     result2 + j * 256, d, 256);
    }
  cout << "avx2 search time" << elapsed() - t0 << endl;
  float average2 = 0.0;
...
void test_float_compute_lookuptable_ip() {
  ...
  for (int i = 0; i < nq; i++) {
    for (int j = 0; j < m; j++) {
      for (int k = 0; k < nb; k++) {
        result[j * 256 + k] = get_ip_gt<float, float, float>(
            xq + i * j * d, xb + (j * 256 + k) * d, d);
      }
    }
  }
  ...
  for (int i = 0; i < nq; i++)
    for (int j = 0; j < m; j++) {
      compute_lookuptable_IP<float>(xq + i * j * d, ytransform + j * 256 * d,
                                    result2 + j * 256, d, 256);
    }
  cout << "avx2 search time" << elapsed() - t0 << endl;
  float average2 = 0.0;
...
void test_float_compute_lookuptable_l2() {
  ...
  for (int i = 0; i < nq; i++) {
    for (int j = 0; j < m; j++) {
      for (int k = 0; k < nb; k++) {
        result[j * 256 + k] = get_l2_gt<float, float, float>(
            xq + i * j * d, xb + (j * 256 + k) * d, d);
      }
    }
  }
  ...
  for (int i = 0; i < nq; i++)
    for (int j = 0; j < m; j++) {
      compute_lookuptable_L2<float>(xq + i * j * d, ytransform + j * 256 * d,
                                    result2 + j * 256, d, 256);
    }
  cout << "avx2 search time" << elapsed() - t0 << endl;
  float average2 = 0.0;
