...
if __name__ == '__main__':
    ...
    total_item_l = np.zeros(shape=n_cluster)
    total_recall_l = np.zeros(shape=n_cluster)
    for i, tmp_query in enumerate(query, 0):
        distance_l = [np.linalg.norm(tmp_query - _) for _ in centroids]
        distance_sort_l = np.argsort(distance_l)
        tmp_set = []
        tmp_set_len = 0
        tmp_item_l = []
        tmp_recall_l = []
        for tmp_idx in distance_sort_l:
            ...
            tmp_set_len += len(cluster_item_l[tmp_idx])
    ...
    for i in range(len(total_item_l)):
        item_recall_l.append({'n_candidate': total_item_l[i], 'recall': total_recall_l[i]})
