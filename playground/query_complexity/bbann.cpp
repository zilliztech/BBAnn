...
bool print = false;
    while (!reti.empty()) {
      *p_labeli++ = reti.top().second;
        { std::cout << i <<"-th query  distance:=" << reti.top().first << " | label: " << reti.top().second << " | p_labeli:=" << p_labeli<< std::endl; }
        reti.pop();
    }
    delete[] queryi;
  }
  ...
  knn_1<CMax<float, uint32_t>, DATAT, float>(
    ...
    for (int i = 0; i < nq; ++i) {
        for (int j = 0; j < nprobe; ++j) {
            std::cout << i <<"-th query" << j << "-th nprobe distance:=" << values[i*nprobe + j]<< std::endl;
        }
    }
    std::cout << "d(q,cen) = [";
    for (int i = 0; i < nq; ++i) {
        for (int j = 0; j < nprobe; ++j) {
            std::cout << values[i*nprobe + j] << ",";
        }
    }
    std::cout << std::endl;
...
#pragma omp parallel for schedule(static, 128)
  ...
  char *buf = new char[block_size];
    ...
    std::unordered_map<uint32_t, uint32_t> bucket_labels_2_id; // bucket_labels->offset in bucket_centroids_file or bucket_centroids_id_file
    for (int64_t i = 0; i < nq; ++i) {
        const auto ii = i * nprobe;
        for (int64_t j = 0; j < nprobe; ++j) {
            bucket_labels_2_id[bucket_labels[ii + j]] = 2;
        }
    }
    {
        std::string bucket_centroids_id_file = index_path + CLUSTER + COMBINE_IDS + BIN;
        std::ifstream bucket_centroids_id_file_f(bucket_centroids_id_file, std::ios::binary);
        assert(input.is_open());
        uint32_t cen_n, cen_dim;
        bucket_centroids_id_file_f.read(reinterpret_cast<char*>(&cen_n), sizeof(cen_n));
        bucket_centroids_id_file_f.read(reinterpret_cast<char*>(&cen_dim), sizeof(cen_dim));
        std::cout << "number of centriods: " << cen_n << std::endl;
        std::cout << "cen_dim: " << cen_dim << std::endl;
        for (size_t i = 0; i < cen_n; ++i) {
            uint32_t cen_id;
            bucket_centroids_id_file_f.read(reinterpret_cast<char*>(&cen_id), sizeof(cen_id));
            if (bucket_labels_2_id.find(cen_id) != bucket_labels_2_id.end()) {
                if (bucket_labels_2_id[cen_id] != 2) {
                    std::cout << "FUCK" << std::endl;
                    return;
                }
                bucket_labels_2_id[cen_id] = i;
            }
        }
    }
    std::string bucket_centroids_file = index_path + BUCKET + CENTROIDS + BIN;
    uint32_t cen_n, cen_dim;
    float *cen_data = nullptr;
    read_bin_file<float>(bucket_centroids_file, cen_data, cen_n, cen_dim);
    std::cout << "number of centriods: " << cen_n << std::endl;
    std::cout << "cen_dim: " << cen_dim << std::endl;
    if (cen_dim != dq) {
        std::cout << "FUCK" << std::endl;
    ...
    /* flat */
std::vector<float> d_v_cen;
std::vector<float> max_d_v_cen;
  for (int64_t i = 0; i < nq; ++i) {
    const auto ii = i * nprobe;
    const DATAT *q_idx = pquery + i * dq;

    for (int64_t j = 0; j < nprobe; ++j) {
{ std::cout << i <<"-th query | " << j << "-th nprobe bucket_labels:=" << bucket_labels[ii + j] << " | centroid ID: " << bucket_labels_2_id[bucket_labels[ii + j]] << std::endl;}
{ std::cout << "CENTRIOD VECTOR := ";}
float* this_cen_data = cen_data + cen_dim * bucket_labels_2_id[bucket_labels[ii + j]];
{ for (int dim_it = 0; dim_it < cen_dim; ++dim_it) std::cout << this_cen_data[dim_it] << ","; std::cout << std::endl; }
      parse_global_block_id(bucket_labels[ii + j], cid, bid);
      std::string cluster_file_path =
          index_path + CLUSTER + std::to_string(cid) + "-" + RAWDATA + BIN;
{ std::cout << i <<"-th query " << j << "-th nprobe cluster_file_path:=" << cluster_file_path << std::endl;}
        auto fh = std::ifstream(cluster_file_path, std::ios::binary);
      assert(!fh.fail());
...
std::vector<float> avg_vec(dim, 0);
std::vector<float> distance_between_vector_CEN;
double max_distance_between_vector = 0.0;
double avg_distance_between_vector = 0.0;
double counter_distance = 0.0;
std::vector<float> angle_between_vector_q;
std::vector<float> distance_between_vector;
std::vector<float> angle_between_vector;
std::vector<float> angle_between_delta_q;
      for (uint32_t k = 0; k < entry_num; ++k) {
        ...
        auto dis = dis_computer(vec, q_idx, dim);
          ...
          {
              // Distance(vec in bucket, Centroiod)
              auto dis_cen_vec = L2sqr<DATAT, float, float>(vec, this_cen_data, dim);
              distance_between_vector_CEN.push_back(dis_cen_vec);
          }
          {
              // IP between vectors in bucket and query
              auto ip = IP<DATAT, const DATAT, float>(vec, q_idx, dim);
              auto vec_norm = std::sqrt(IP<DATAT, DATAT, float>(vec, vec, dim));
              auto q_norm = std::sqrt(IP<const DATAT, const DATAT, float>(q_idx, q_idx, dim));
              auto arccos = ip / (vec_norm * q_norm);
              angle_between_vector_q.emplace_back(std::acos(arccos));
          }
        for (uint32_t k_it = k + 1; k_it < entry_num; ++k_it) {
            char *entry_begin_other = buf_begin + entry_size * k_it;
            auto vec_other = reinterpret_cast<DATAT *>(entry_begin_other);
            auto local_dis = dis_computer(vec, vec_other, dim);
            distance_between_vector.emplace_back(local_dis);
            if (local_dis > max_distance_between_vector) max_distance_between_vector = local_dis;
            avg_distance_between_vector += local_dis;
            counter_distance += 1.0;

            {
                // IP between vectors in bucket
                auto ip = IP<DATAT, DATAT, float>(vec, vec_other, dim);
                auto vec_norm = std::sqrt(IP<DATAT, DATAT, float>(vec, vec, dim));
                auto vec_other_norm = std::sqrt(IP<DATAT, DATAT, float>(vec_other, vec_other, dim));
                auto arccos = ip / (vec_norm * vec_other_norm);
                angle_between_vector.emplace_back(std::acos(arccos));
            }

            {
                // IP between delta of vectors and query
                int* delta_vector = new int[(uint64_t)dim];
                std::cout << "delta_vector = [";
                for (int cen_dim_it = 0; cen_dim_it < cen_dim; ++cen_dim_it) { delta_vector[cen_dim_it] = vec[cen_dim_it] - vec_other[cen_dim_it]; std::cout << static_cast<int>(delta_vector[cen_dim_it]) << ","; }
                std::cout << std::endl;
                auto ip = IP<int, const DATAT, float>(delta_vector, q_idx, dim);
                auto vec_norm = std::sqrt(IP<int, int, float>(delta_vector, delta_vector, dim));
                auto q_norm = std::sqrt(IP<const DATAT, const DATAT, float>(q_idx, q_idx, dim));
                auto arccos = ip / (vec_norm * q_norm);
                angle_between_delta_q.emplace_back(std::acos(arccos));
            }
        }
{ std::cout << k << "-th entry's VECTOR := ";}
{for (int dim_it = 0; dim_it < dim; ++dim_it) { std::cout << static_cast<int>(vec[dim_it]) << ","; avg_vec[dim_it] += vec[dim_it]; } std::cout << std::endl; }
{ std::cout << k << "-th entry's DISTANCE := " << dis << std::endl;}
...
{ std::cout << "angle_between_vector_q = [";}
{ for (auto& ele : angle_between_vector_q) std::cout << ele << ","; std::cout << std::endl; }
{ std::cout << "distance_between_vector = [";}
{ for (auto& ele : distance_between_vector) std::cout << ele << ","; std::cout << std::endl; }
{ std::cout << "distance_between_vector_CEN = [";}
{ for (auto& ele : distance_between_vector_CEN) std::cout << ele << ","; std::cout << std::endl; }
float r = std::accumulate(distance_between_vector_CEN.begin(), distance_between_vector_CEN.end(), 0.0) / static_cast<float>(distance_between_vector_CEN.size());
d_v_cen.push_back(r);
float max = std::max_element(distance_between_vector_CEN.begin(), distance_between_vector_CEN.end());
max_d_v_cen.push_back(max);
{ std::cout << "angle_between_vector = [";}
{ for (auto& ele : angle_between_vector) std::cout << ele << ","; std::cout << std::endl; }
{ std::cout << "angle_between_delta_q = [";}
{ for (auto& ele : angle_between_delta_q) std::cout << ele << ","; std::cout << std::endl; }
{ for (auto& ele : avg_vec) ele /= static_cast<float>(entry_num); }
{ std::cout << "AVG VECTOR := ";}
{ for (auto& ele : avg_vec) std::cout << ele << ","; std::cout << std::endl; }
std::vector<DATAT> avg_vec_(avg_vec.begin(), avg_vec.end());
auto dis_2_avg = dis_computer(avg_vec_.data(), q_idx, dim);
{ std::cout << "DISTANCE(query, avg_vec) := " << dis_2_avg << std::endl;}
{ std::cout << "max_distance_between_vector := " << max_distance_between_vector << std::endl;}
{ std::cout << "avg_distance_between_vector := " << avg_distance_between_vector / counter_distance << std::endl;}


        std::vector<float> angle_between_centroid_vec;
        std::vector<float> angle_between_centroid_q;
        std::vector<float> distance_between_vector_AVGCEN;
        for (uint32_t k = 0; k < entry_num; ++k) {
            char *entry_begin = buf_begin + entry_size * k;
            vec = reinterpret_cast<DATAT *>(entry_begin);
            {
                // Distance(vec in bucket, AVG Centroiod)
                auto dis_cen_vec = L2sqr<DATAT, DATAT, float>(vec, avg_vec_.data(), dim);
                distance_between_vector_AVGCEN.push_back(dis_cen_vec);
            }
            {
                // IP between vectors in bucket and AVG centroid
                auto ip = IP<DATAT, DATAT, float>(vec, avg_vec_.data(), dim);
                auto vec_norm = std::sqrt(IP<DATAT, DATAT, float>(vec, vec, dim));
                auto cen_norm = std::sqrt(IP<DATAT, DATAT, float>(avg_vec_.data(), avg_vec_.data(), dim));
                auto arccos = ip / (vec_norm * cen_norm);
                angle_between_centroid_vec.emplace_back(std::acos(arccos));
            }
            {
                // IP between vectors in bucket and AVG centroid
                auto ip = IP<const DATAT, DATAT, float>(q_idx, avg_vec_.data(), dim);
                auto q_norm = std::sqrt(IP<const DATAT, const DATAT, float>(q_idx, q_idx, dim));
                auto cen_norm = std::sqrt(IP<DATAT, DATAT, float>(avg_vec_.data(), avg_vec_.data(), dim));
                auto arccos = ip / (q_norm * cen_norm);
                angle_between_centroid_q.emplace_back(std::acos(arccos));
            }
        }
        { std::cout << "distance_between_vector_AVGCEN = [";}
        { for (auto& ele : distance_between_vector_AVGCEN) std::cout << ele << ","; std::cout << std::endl; }
        { std::cout << "angle_between_AVGcentroid_vec = [";}
        { for (auto& ele : angle_between_centroid_vec) std::cout << ele << ","; std::cout << std::endl; }
        { std::cout << "angle_between_AVGcentroid_q = [";}
        { for (auto& ele : angle_between_centroid_q) std::cout << ele << ","; std::cout << std::endl; }

    }
  }

    { std::cout << "radius = [";}
    { for (auto& ele : d_v_cen) std::cout << ele << ","; std::cout << std::endl; }
    { std::cout << "max_radius = [";}
    { for (auto& ele : max_d_v_cen) std::cout << ele << ","; std::cout << std::endl; }
