...
//                    if (candidate_id == 0) continue;
  ...
  std::vector<std::pair<dist_t, labeltype>> getNeighboursWithinRadius(
      std::priority_queue<std::pair<dist_t, tableint>,
                          std::vector<std::pair<dist_t, tableint>>,
                          CompareByFirst> &top_candidates,
      const void *data_point, float radius) const {
    ...
    while (!top_candidates.empty()) {
      ...
      if (cand.first < radius) {
        ...
        result.emplace_back(cand.first, getExternalLabel(cand.second));
      }
      ...
      for (size_t j = 1; j <= size; j++) {
        ...
        if (!(visited_array[candidate_id] == visited_array_tag)) {
          ...
          if (dist < radius) {
            ...
            result.emplace_back(dist, getExternalLabel(candidate_id));
          }
        }
      }
    }
  ...
  std::priority_queue<std::pair<dist_t, tableint>,
                      std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
  searchBaseLayerRange(tableint ep_id, const void *data_point, size_t ef,
                       float radius) const {
    VisitedList *vl = visited_list_pool_->getFreeVisitedList();
    vl_type *visited_array = vl->mass;
    vl_type visited_array_tag = vl->curV;
      ...
      for (size_t j = 1; j <= size; j++) {
        ...
        if (!(visited_array[candidate_id] == visited_array_tag)) {
          ...
          if (top_candidates.size() < ef || lowerBound > dist) {
            ...
            if (!top_candidates.empty())
              lowerBound = std::max(top_candidates.top().first, dist);
          }
        }
      }
    }
  ...
  std::vector<tableint> getNeighborsByHeuristic2(
      std::priority_queue<std::pair<dist_t, tableint>,
                          std::vector<std::pair<dist_t, tableint>>,
                          CompareByFirst> &top_candidates,
      const size_t M) {
      ...
      for (std::pair<dist_t, tableint> &current_pair : queue_closest) {
        ...
        for (tableint id : return_list) {
          dist_t curdist = fstdistfunc_(
              getDataByInternalId(id), getDataByInternalId(current_pair.second),
              dist_func_param_);
    ...
    for (size_t idx = 0; idx < selectedNeighbors.size(); idx++) {
      ...
      if (!is_cur_c_present) {
        if (sz_link_list_other < Mcurmax) {
          ...
          setListCount(ll_other,
                       static_cast<unsigned short int>(selected.size()));
  ...
  searchRange(const void *query_data, size_t k, float radius) const {
    ...
    return getNeighboursWithinRadius(top_candidates, query_data, radius);
  }
