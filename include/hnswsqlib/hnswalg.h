...
namespace hnswsqlib {
    ...
    public:
        ...
        int maxlevel_;
        ScalarQuantizer *scalar_quantizer_;
        ...
        void train_codebook(float * data, int64_t n) {
            scalar_quantizer_->train_codebook(data, n);
        }
...
//                    if (candidate_id == 0) continue;
        ...
        void saveIndex(const std::string &location) {
            ...
            std::ofstream codes_output(location + ".codebook", std::ios::binary);
            codes_output.write((char*)scalar_quantizer_->get_codebook(), scalar_quantizer_->get_codebook_size());
            codes_output.close();
        }
