...
struct refine_stat {
  ...
  refine_stat()
      : vector_page_hit_cnt(0), vector_load_cnt(0), id_page_hit_cnt(0),
        id_load_cnt(0), different_offset_cnt(0) {}
};

template <typename T>
inline void stat_length(T *x, int64_t n, int64_t dim, double &max_len,
                        double &min_len, double &avg_len) {
  double sum_len = 0;
  max_len = 0;
