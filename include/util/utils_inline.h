...
inline void load_meta_impl(const std::string &index_path,
                           ...
                           const int K1) {
  for (int i = 0; i < K1; i++) {
    std::ifstream reader(index_path + CLUSTER + std::to_string(i) + META + BIN,
                         std::ios::binary);
    ...
    assert(1 == dmeta);
    meta[i].resize(nmeta);
    reader.read((char *)meta[i].data(),
                (uint64_t)nmeta * dmeta * sizeof(uint32_t));
    reader.close();
  }
}
*/
...
inline void write_bin_file(const std::string& file_name, T* data, uint32_t n,
    ...
    writer.close();
    std::cout << "write binary file to " << file_name << " done in ... seconds,
n = "
              << n << ", dim = " << dim << std::endl;
}
...
inline void read_bin_file(const std::string& file_name, T*& data, uint32_t& n,
    ...
    reader.close();
    std::cout << "read binary file from " << file_name << " done in ... seconds,
n = "
              << n << ", dim = " << dim << std::endl;
}
...
T* sample_data) { assert(sample_data != nullptr); std::random_device rd; auto x
= rd(); std::mt19937 generator((unsigned) x); uint32_t nb, dim; size_t ntotal,
ndims; IOReader reader(data_file); reader.read((char*)&nb, sizeof(uint32_t));
    reader.read((char*)&dim, sizeof(uint32_t));
    ntotal = nb;
    ndims = dim;
    ...
    for (size_t i = sample_num; i < ntotal; i ++) {
        ...
        if (rand < sample_num) {
            memcpy((char*)(sample_data + ndims * rand), tmp_buf.get(), ndims *
sizeof(T));
        }
    }
}
...
void reservoir_sampling_residual(
    ...
    std::string data_file = output_path + CLUSTER + std::to_string(i) + RAWDATA
+ BIN;
    ...
    for (size_t j = 0; j < metas[i].size(); ++j) {
        for (uint32_t k = 0; k < metas[i][j]; ++k) {
            ...
            if (global_cnt < sample_num) {
                memcpy(sample_data + cluster_dim * global_cnt, vec, cluster_dim
* sizeof(T)); ivf_cen_offsets[global_cnt] = j; } else {
                std::uniform_int_distribution<size_t> distribution(0,
global_cnt); size_t rand = (size_t)distribution(generator); if (rand <
sample_num) { memcpy(sample_data + cluster_dim * rand, vec, cluster_dim *
sizeof(T)); ivf_cen_offsets[rand] = j;
                }
            }
...
inline uint64_t gen_id(const uint32_t cid, const uint32_t bid, const uint32_t
off) { uint64_t ret = 0; ret |= (cid & 0xff); ret <<= 24; ret |= (bid &
0xffffff); ret <<= 32; ret |= (off & 0xffffffff); return ret;
}
...
inline uint64_t gen_refine_id(const uint32_t cid, const uint32_t offset, const
uint32_t queryid) { uint64_t ret = 0; ret |= (cid & 0x000000ff); ret <<= 32; ret
|= (offset & 0xffffffff); ret <<= 24; ret |= (queryid & 0x00ffffff); return ret;
}

inline void parse_refine_id(uint64_t id, uint32_t& cid, uint32_t& offset,
uint32_t& queryid) { queryid = (id & 0x00ffffff); id >>= 24; offset = (id &
0xffffffff); id >>= 32; cid = (id & 0x000000ff);
}
...
void print_vec_id_dis(std::vector<std::vector<std::pair<IDT, DISTT>>>& v, const
std::string msg) { std::cout <<
"<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" << std::endl; std::cout <<
msg << std::endl; for (auto i = 0; i < v.size(); i ++) { for (auto j = 0; j <
v[i].size(); j ++) { std::cout << "(" << v[i][j].first << ", " << v[i][j].second
<< ") ";
        }
        std::cout << std::endl;
    }
...
template<typename DISTT, typename IDT>
void read_sift(std::vector<std::vector<std::pair<IDT, DISTT>>>& v, const
std::string& gt_file, uint32_t& nq, uint32_t& topk) { nq = 100; // default value
...
void read_comp(std::vector<std::vector<std::pair<IDT, DISTT>>>& v, const
std::string& gt_file, uint32_t& nq, uint32_t& topk) { std::ifstream gin(gt_file,
std::ios::binary);
...
template<typename DISTT, typename IDT>
void recall(const std::string& groundtruth_file, const std::string& answer_file,
MetricType metric_type, bool use_comp_format = true, bool cmp_id = false) {
    ...
    if (use_comp_format) {
        read_comp<float, uint32_t, DISTT, IDT>(groundtruth, groundtruth_file,
gt_nq, gt_topk); } else { read_sift<DISTT, IDT>(groundtruth, groundtruth_file,
gt_nq, gt_topk);
    }
    ...
    if (use_comp_format) {
        read_comp<DISTT, IDT, DISTT, IDT>(resultset, answer_file, answer_nq,
answer_topk); } else { read_sift<DISTT, IDT>(resultset, answer_file, answer_nq,
answer_topk);
    }

    if (gt_nq != answer_nq || gt_topk < answer_topk) { // || gt_topk !=
answer_topk) { std::cerr << "Grountdtruth parammeters does not match. GT nq " <<
gt_nq
        << "(" << answer_nq << "), topk " << gt_topk << "(" << answer_topk <<
")" << std::endl; return ;
    }
    ...
    // recall statistics
    std::vector<int> border = {0, 10, 20, 30, 40, 50, 60, 70, 80, 85, 90, 95,
100}; int recall0 = 0; int recall100 = 0; std::vector<int>
recall_hist(border.size(), 0); std::vector<int> recalls(answer_nq);
    ...
    if (cmp_id) {
        ...
        std::cout << "avg recall@" << answer_topk << " = " <<
((double)(tot_cnt)) / answer_topk / answer_nq * 100 << "%." << std::endl; } else
{ if (MetricType::L2 == metric_type) { for (auto i = 0; i < answer_nq; i ++) {
                int cnti = 0;
                for (auto j = 0; j < answer_topk; j ++) {
                    if (resultset[i][j].second <= groundtruth[i][answer_topk -
1].second) cnti ++;
                }
                recalls[i] = cnti;
                tot_cnt += cnti;
                // std::cout << "query " << i << " recall@" << answer_topk << "
is: " << ((double)(cnti)) / answer_topk * 100 << "%." << std::endl;
            }
            std::cout << "avg recall@" << answer_topk << " = " <<
((double)(tot_cnt)) / answer_topk / answer_nq * 100 << "%." << std::endl; } else
if (MetricType::IP == metric_type) { for (auto i = 0; i < answer_nq; i ++) { int
cnti = 0; for (auto j = 0; j < answer_topk; j ++) { if (resultset[i][j].second
>= groundtruth[i][answer_topk - 1].second) cnti ++;
                }
                recalls[i] = cnti;
                tot_cnt += cnti;
                // std::cout << "query " << i << " recall@" << answer_topk << "
...
{ std::cout << "invalid metric type: " << (int)metric_type << std::endl;
    ...
    for (auto i = 0; i < answer_nq; i ++) {
        ...
        for (auto j = 0; j < border.size() - 1; j ++) {
            if (recalls[i] * 100 >= border[j] * answer_topk && recalls[i] * 100
< border[j + 1] * answer_topk) { recall_hist[j] ++; break;
            }
        }
    }
    int check_sum = recall0 + recall100;
    std::cout << "show more details about recall histogram:" << std::endl;
    std::cout << "recall@" << answer_topk << " in range [0, 0]: " << recall0 <<
std::endl; for (auto i = 0; i < border.size() - 1; i ++) { std::cout <<
"recall@" << answer_topk << " in range [" << border[i] << ", " <<  border[i + 1]
<< "): " << recall_hist[i] << std::endl; check_sum += recall_hist[i];
    }
    std::cout << "recall@" << answer_topk << " in range [100, 100]: " <<
recall100 << std::endl; std::cout << "check sum recall: " << check_sum << ",
which should equal nq: " << answer_nq << std::endl;
}
