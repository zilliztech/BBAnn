...
void ssk_init_assign(int64_t nx,
                 int64_t k,
                 uint64_t max_target_size,
                 const float* dis_tab,
                 int64_t* hassign,
                 int64_t* assign) {
    uint64_t remain = nx;
    ...
    while (remain) {
        ...
        std::sort(points[idx].begin(), points[idx].begin() + remain, [&](const auto& x, const auto& y) {
    ...
    for (int64_t iter = 0; iter < niter; ++iter) {
        ...
        for (int64_t i = 0; i < nx; ++i) {
            ...
            for (int64_t j = 0; j < k; ++j) {
                ...
                if (x_gain > 0 && (hassign[x_cluster] > min_target_size && hassign[j] < max_target_size)) {
                    --hassign[x_cluster];
                    ++hassign[j];
                    assign[x] = j;
                    x_cluster = j;
            ...
            if (assign[x] != ks[0] && dis_tab[x*k+assign[x]] > dis_tab[x*k+ks[0]]) {
