...
namespace sq_hnswlib {
    template<typename T>
    uint8_t *
    train_codebook(const std::string& filename,
                   const int hnswM,
                   const int hnswefC,
                   MetricType metric_type,
                   std::function<void(const std::string&, uint32_t&, uint32_t&)> read_meta,
                   std::function<const std::string&, T*&, uint32_t&, uint32_t&, const uint32_t&> read_bin_file_half_dimension,
                   std::function<const T*, const int64_t, const int64_t, const int64_t, float*&> kmeans
                   ) {
