  ...
  searchBaseLayer(tableint ep_id, const void *data_point, int layer,
    ...
    if (!isMarkedDeleted(ep_id)) {
      dist_t dist = fstdistfunc_(data_point, getDataByInternalId(ep_id),
                                 dist_func_param_, codes_, both_codes);
...
//                    if (candidate_id == 0) continue;
  ...
  searchKnnInternal(void *query_data, int k) {
    ...
    if (has_deletions_) {
      std::priority_queue<std::pair<dist_t, tableint>> top_candidates1 =
          searchBaseLayerST<true>(currObj, query_data, ef_, codes_, false);
      top_candidates.swap(top_candidates1);
    } else {
      std::priority_queue<std::pair<dist_t, tableint>> top_candidates1 =
          searchBaseLayerST<false>(currObj, query_data, ef_, codes_, false);
      top_candidates.swap(top_candidates1);
    }
  ...
  void saveIndex(const std::string &location) {
    ...
    codes_output.write((char *)codes_,
                       sizeof(float) * 256 * (*(size_t *)dist_func_param_));
    codes_output.close();
  }
  ...
  void loadIndex(const std::string &location, SpaceInterface<dist_t> *s,
    ...
    codes_ =
        (float *)malloc(sizeof(float) * 256 * (*(size_t *)dist_func_param_));
    std::ifstream codes_input(location + ".codes", std::ios::binary);
    codes_input.read((char *)codes_,
                     sizeof(float) * 256 * (*(size_t *)dist_func_param_));
    codes_input.close();
  ...
  void repairConnectionsForUpdate(const void *dataPoint,
    ...
    for (int level = dataPointLevel; level >= 0; level--) {
      std::priority_queue<std::pair<dist_t, tableint>,
                          std::vector<std::pair<dist_t, tableint>>,
                          CompareByFirst>
          topCandidates = searchBaseLayer(currObj, dataPoint, level, true);
    ...
    if ((signed)currObj != -1) {
      ...
      for (int level = std::min(curlevel, maxlevelcopy); level >= 0; level--) {
        ...
        std::priority_queue<std::pair<dist_t, tableint>,
                            std::vector<std::pair<dist_t, tableint>>,
                            CompareByFirst>
            top_candidates = searchBaseLayer(currObj, data_point, level, true);
        if (epDeleted) {
