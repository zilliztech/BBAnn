...
sed -e 's/IDENTIFIER/M32/g' ../python/bbann-algo-1B-template.yaml | sed -e 's/HNSWM/32/g' > ../python/bbann-algo-docker.yaml
rm -rf results/ssnpp-1B*
python3 run.py --definitions ../python/bbann-algo-docker.yaml --dataset ssnpp-1B  --algorithm bbann --runs 1 --count 96237  --force # --rebuild
python3 plot.py --definitions ../python/bbann-algo-docker.yaml --dataset ssnpp-1B --recompute --count 96237
popd
