...
sed -e 's/BLOCKSIZE/8192/g' ../python/bbann-algo-1B-template.yaml  | sed -e 's/INDEXPATH/"\/data\/BBANN-SSNPP-24-500-128-2"/g' > ../python/bbann-algo-4K.yaml 
rm -rf results/ssnpp-1B*
python3 run.py --definitions ../python/bbann-algo-4K.yaml --nodocker --dataset ssnpp-1B --algorithm bbann --runs 1 --count 96237 # --force --rebuild
