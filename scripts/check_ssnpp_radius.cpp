...
std::string q_path =
    "/home/pat/datasets/Facebook-SimSearchNet++/FB_ssnpp_public_queries.u8bin";
std::string base_path =
    "/home/pat/datasets/Facebook-SimSearchNet++/FB_ssnpp_database.10M.u8bin";
...
int main() {
  ...
  for (auto i = 0; i < nq; ++i) {
    ...
    for (const auto &id : v[i]) {
      ...
      auto dis = L2sqr<const uint8_t, const uint8_t, uint32_t>(
          q, reinterpret_cast<uint8_t *>(vec), dim);
