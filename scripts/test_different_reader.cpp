...
/*
    // mmap
    {
        ...
        xb = (unsigned char*)mmap(NULL, (long long)nb * dim + 8, PROT_READ, MAP_SHARED, fd, 0);
#pragma omp parallel for reduction(max:max_len) reduction(min:min_len)    
        for (int i=0;i<nq;i++){
            unsigned char *tmp = xb + ((long long)xq[i] * dim + 8);
            int len = IP<unsigned char,unsigned char,int>(tmp,tmp,dim);
            if(max_len<len) {max_len=len;}
            if(min_len>len) {min_len=len;}
        }
...
#pragma omp parallel for
        for(int j = 0; j < batch_num;j++){
            io_prep_pread(p_cb[j], fd, xb+j*dim, dim, (long long)(xq[i+j]) * dim + 8);
        }
