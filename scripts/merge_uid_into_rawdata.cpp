...
int main(int argc, char **argv) {
  if (argc != 5) {
    std::cout << "Usage:" << argv[0] << " data type(float or uint8 or int8)"
              << " index path"
              << " cluster number"
              << " dimension of data" << std::endl;
    return 1;
  }
  ...
  } else if (std::string("uint8") == argv[1] ||
             std::string("int8") == argv[1]) {
    vector_size *= 1;
  } else {
    std::cout << "wrong data type, must be float or uint8 or int8!"
              << std::endl;
    return 1;
  }
  ...
  for (auto i = 0; i < cluster_num; i++) {
    std::string aligned_data_filei =
        index_path + "cluster-" + "-" + std::to_string(i) + "raw_data" + ".bin";
    std::string aligned_ids_filei = index_path + "cluster-" + "-" +
                                    std::to_string(i) + "global_ids" + ".bin";
    std::string aligned_data_filei2 =
        index_path + "cluster-" + std::to_string(i) + "-" + "raw_data" + ".bin";
    raw_data_file_handlers[i] =
        std::ifstream(aligned_data_filei, std::ios::binary);
    ids_data_file_handlers[i] =
        std::ifstream(aligned_ids_filei, std::ios::binary);
    ...
    *(uint32_t *)(node_buf + 5 * sizeof(uint32_t)) = i;
    ...
    for (int j = 1; j < npn; j++) {
      ...
      for (int k = 0; k < nnpp && write_cnt < clu_size; k++) {
        ...
        if (0 == write_cnt % nipp)
          ids_data_file_handlers[i].read(ids_buf, page_size);
        memcpy(node_buf + k * node_size,
               dat_buf + (write_cnt % nvpp) * vector_size, vector_size);
        memcpy(node_buf + k * node_size + vector_size,
               ids_buf + (write_cnt % nipp) * id_size, id_size);
        write_cnt++;
      }
      data_writer[i].write(node_buf, page_size);
    }
